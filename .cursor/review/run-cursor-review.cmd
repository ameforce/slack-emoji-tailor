...
if not defined REVIEW_CURSOR_MODEL_PRIMARY set "REVIEW_CURSOR_MODEL_PRIMARY=gpt-5.3-codex-xhigh"
if not defined REVIEW_CURSOR_MODEL_FALLBACK set "REVIEW_CURSOR_MODEL_FALLBACK=gpt-5.3-codex-spark-preview"
...
set "RAW_PRIMARY=%OUT_DIR%\cursor-primary.txt"
set "RAW_FALLBACK=%OUT_DIR%\cursor-fallback.txt"
...
pushd "%REPO_ROOT%" >nul
agent --print --output-format=text --model "%REVIEW_CURSOR_MODEL_PRIMARY%" < "%PROMPT_FILE%" > "%RAW_PRIMARY%" 2> "%LOG_FILE%"
set "RC=%ERRORLEVEL%"
if "%RC%"=="0" goto :success_primary

echo Primary cursor run failed with exit code %RC%.>> "%LOG_FILE%"
agent --print --output-format=text --model "%REVIEW_CURSOR_MODEL_FALLBACK%" < "%PROMPT_FILE%" > "%RAW_FALLBACK%" 2>> "%LOG_FILE%"
set "RC=%ERRORLEVEL%"
if "%RC%"=="0" goto :success_fallback

popd >nul
> "%OUT_DIR%\review.fail" echo cursor_failed
...
>> "%OUT_DIR%\review.log" echo backend=cursor model=%REVIEW_CURSOR_MODEL_PRIMARY%
> "%OUT_DIR%\review.done" echo cursor_primary
del /q "%OUT_DIR%\review.running" 2>nul
exit /b 0
...
>> "%OUT_DIR%\review.log" echo backend_order=%BACKEND_ORDER%
>> "%OUT_DIR%\review.log" echo backend=cursor model=%REVIEW_CURSOR_MODEL_FALLBACK%
> "%OUT_DIR%\review.done" echo cursor_fallback
del /q "%OUT_DIR%\review.running" 2>nul
exit /b 0
