...
def test_derive_display_version_keeps_commits_ahead_from_git_describe() -> None:
    assert versioning.derive_display_version_from_describe("v0.1.1-2-gaaed0c6") == "v0.1.1-2-gaaed0c6"
...
def test_get_display_version_uses_git_describe_even_when_env_is_set(
    ...
    monkeypatch.setattr(versioning, "_baked_git_tag_version", lambda: "v0.1.0")

    assert versioning.get_display_version() == "v0.1.1-2-gaaed0c6"
...
def test_get_display_version_reads_baked_git_tag_version_when_git_is_unavailable(
    ...
    monkeypatch.setattr(versioning, "_baked_git_tag_version", lambda: "v0.1.1-4-gce9e3e1")

    assert versioning.get_display_version() == "v0.1.1-4-gce9e3e1"
...
def test_get_display_version_is_cached(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert versioning.get_display_version() == "v0.1.1-1-gaaed0c6"
    assert versioning.get_display_version() == "v0.1.1-1-gaaed0c6"
    assert calls == 1
